...
The public form submits to `/api/leads` on the same website domain. The production domains are:
...
Use `https://tagagency.in` as the canonical public URL. The older Cloudflare Workers preview URL is no longer canonical.
...
Email DNS remains separate from this website deployment. Do not change MX, SPF, DMARC, autoconfig, autodiscover, or other email DNS records while updating landing-page hosting.
